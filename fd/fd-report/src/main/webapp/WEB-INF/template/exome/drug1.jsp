...
<div class="jumbotron">
    ...
    <span class="subtitle">${sense.senName}药物风险简介：</span>
    <div class="acticle">
...
<div class="wrap re-wrap">
    ...
    <table class="title-garden-table table1" cellspacing="0" cellpadding="0">
        <thead>
            <tr><th>药物/疗法</th><th>检测基因</th><th>检测位点</th><th>级别</th><th>基因型</th><th>用药提示</th></tr>
        </thead>
        <tbody>
            <c:forEach items="${cheDrugGeneList }" var="temp">
            <tr><td>${temp.drug}</td><td class="weidian"><a href="http://www.genecards.org/cgi-bin/carddisp.pl?gene=${temp.gene}" target="_blank" onclick="return (function(){var truthBeTold = window.confirm('你正链接到genecards...');  if(truthBeTold==false){return false;}})()">${temp.gene}</a></td> 
           	<c:choose>
           		<c:when test="${fn:contains(temp.locus, '[PMID:') }">
           			<td class="nl">${fn:substringBefore(temp.locus, '[PMID:')}<br/>[PMID:${fn:substringAfter(temp.locus, '[PMID:')}</td>
           		</c:when>
           		<c:otherwise>
           			<td class="nl">${temp.locus}</td>
           		</c:otherwise>
           	</c:choose>
